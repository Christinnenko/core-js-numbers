...
function getAverage(value1, value2) {
  if (!Number.isFinite(value1) || !Number.isFinite(value2)) {
    // Если одно из значений не является конечным числом, возвращаем NaN
    return NaN;
  }

  const result = (value1 + value2) / 2;

  // Возвращаем Number.MAX_VALUE, если результат бесконечен
...
function getLinearEquationRoot(a, b) {
  // Проверяем, что a не равно 0 (чтобы избежать деления на 0)
  if (a === 0) {
    throw new Error("Coefficient 'a' must not be zero for a linear equation.");
  }

  // Решаем уравнение и возвращаем корень
  const root = -b / a;
  return root;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // Возвращаем угол между векторами в радианах
...
function isPrime(n) {
  ...
  if (n % 2 === 0 || n % 3 === 0) {
    return false; // Если число делится нацело на 2 или 3, то оно не простое
  }

  // Проверяем делители, начиная с 5 и идя по шагу 6 (5, 11, 17, 23, ...)
...
function toNumber(value, def) {
  const convertedValue = +value; // Используем унарный плюс для преобразования в число
...
function isPowerOfTwo(num) {
  ...
  // Using the bitwise AND operation with (num - 1) will result in 0 for power of two numbers.
...
function getCountOfOddNumbers(number) {
  const num2 = Math.abs(number);
  const oddCount = Array.from({ length: num2 + 1 }, (_, i) => i).filter(
    (num) => num % 2 !== 0
  ).length;
